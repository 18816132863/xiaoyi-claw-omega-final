...
class NewsSearchAPI:
    ...
    def search(self, query: str, max_retries: int = 3, call_type: str = "normal") -> List[Dict[str, Any]]:
        """
        搜索财经资讯
    ...
    def batch_search(self, queries: List[str], delay: float = 1.0, call_type: str = "normal") -> Dict[str, List[Dict[str, Any]]]:
        """
        批量搜索
        ...
        for i, query in enumerate(queries):
            try:
                ...
                articles = self.search(query, call_type=call_type)
...
问财OpenAPI网关规范:
1. 所有请求包含X-Claw-* HTTP头
2. 使用Bearer Token认证，从IWENCAI_API_KEY环境变量读取
3. 每次请求生成64字符唯一Trace ID
4. 支持跨平台使用

环境变量设置:
  Unix/Linux/macOS: export IWENCAI_API_KEY="your_api_key"
  Windows PowerShell: $env:IWENCAI_API_KEY="your_api_key"
  Windows CMD: set IWENCAI_API_KEY=your_api_key
        """
    )
