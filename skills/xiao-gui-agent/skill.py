#!/usr/bin/env python3
"""
xiao-gui-agent 技能执行脚本
通过模拟人在手机屏幕上的操作（点击、滑动、输入、拖拽等）自动完成手机APP中的任务。当任务需要真实操作手机APP界面、获取APP内部信息、执行用户行为（搜索、签到、关注、购买、发布等）时，应使用该Skill。适用于所有需要在手机上直接操控APP完成的指令。
"""
