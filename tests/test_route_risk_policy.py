...
class RiskPolicyValidator:
    ...
    def get_system_routes(self) -> List[str]:
        ...
        return [
            route_id for route_id, route in self.routes.items()
            if route.get("risk_level") == "SYSTEM"
        ]
    ...
    def validate_risk_policy(self, route_id: str) -> Dict:
        ...
        # 高风险和系统级路由必须需要确认
        if risk_level in ["HIGH", "SYSTEM"] and not requires_confirmation:
            errors.append(f"{risk_level} risk route should require confirmation")
        
        # 删除操作应该是高风险
        if "delete" in route_id.lower() and risk_level != "HIGH":
            errors.append("Delete operation should be HIGH risk")
        
        # 查询操作应该是低风险
        if "query" in route_id.lower() and risk_level not in ["LOW", "MEDIUM"]:
            errors.append("Query operation should be LOW or MEDIUM risk")
    ...
    def get_risk_distribution(self) -> Dict[str, int]:
        """获取风险分布"""
        distribution = {"LOW": 0, "MEDIUM": 0, "HIGH": 0, "SYSTEM": 0}
...
class TestRiskDistribution:
    ...
    def test_risk_distribution_reasonable(self, risk_validator):
        ...
        # 应该有各种风险等级
        assert distribution["LOW"] > 0, "应该有低风险路由"
        assert distribution["HIGH"] > 0, "应该有高风险路由"
...
class TestRiskPolicyValidation:
    ...
    def test_confirmation_count_matches(self, risk_validator):
        ...
        stats = risk_validator.registry.get("stats", {}).get("requires_confirmation", 0)
...
class TestRiskPolicyConsistency:
    ...
    def test_similar_operations_same_risk(self, risk_validator):
        ...
        assert risk_levels == {"HIGH"}, \
            f"所有删除操作应该是高风险，实际: {risk_levels}"
    ...
    def test_create_vs_delete_different_risk(self, risk_validator):
        ...
        if create_routes and delete_routes:
            avg_create_risk = sum(
                {"LOW": 0, "MEDIUM": 1, "HIGH": 2, "SYSTEM": 3}.get(
                    risk_validator.routes[route_id]["risk_level"], 0
                )
